    ...
    def createHost(self, NUMBER):
        ...
        for i in range(1, NUMBER + 1):
            if i >= 100:
                ...
                PREFIX = "h00"
            # 注意：cpu 参数需要使用 CPULimitedHost 才会生效；此处保留原逻辑
...
def install_proactive(net, topo):
    ...
    for sw in topo.EdgeSwitchList:
        ...
        Edge_List = [i for i in range(1, 1 + topo.pod ** 2 // 2)]
        for i in Edge_List:
            if i != num:
                for j in range(1, topo.pod // 2 + 1):
                    for k in range(1, topo.pod // 2 + 1):
                        cmd = "ovs-ofctl add-flow %s -O OpenFlow13 \
                        'table=0,idle_timeout=0,hard_timeout=0,priority=10,arp,\
...
if __name__ == '__main__':
    ...
    if os.getuid() != 0:
        ...
        # 同机控制器：默认 127.0.0.1:6633；如你的 Ryu 用 6653，请改 port=6653
        createTopo(4, 2)
        # createTopo(8, 4)
